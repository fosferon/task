    ...
    createToolFunction(
      ...
      'Report that the task has completed successfully',
      {
        result: {
          type: 'string',
          description:
            'A few paragraphs describing the result. Be thorough and comprehensive.',
        },
      },
      undefined,
      'task_complete',
...
async function processMetaMemory(
  ...
  if (taskLocalState.memory?.processing) {
    ...
    if (processingTime > STUCK_THRESHOLD) {
      console.warn(
        `[Task] Meta memory processing appears stuck (running for ${Math.round(processingTime / 1000)}s), forcibly resetting`,
      );
  ...
  console.log(
    `[Task] Processing ${unprocessedMessages.length} messages for meta memory`,
  );
  ...
  if (taskLocalState.memory.state) {
    ...
    startEventData.state = {
      ...
      taggedMessages: Object.fromEntries(
        taskLocalState.memory.state.taggedMessages,
      ),
  ...
  const timeoutPromise = new Promise<void>((_, reject) => {
    const timeoutId = setTimeout(
      () => reject(new Error('Metamemory processing timeout after 3 minutes')),
      MEMORY_TIMEOUT,
    );
  ...
  const memoryOp = Promise.race([memoryPromise, timeoutPromise])
    .then(async (result) => {
      ...
      if (taskLocalState?.memory) {
        ...
        console.log(
          `[Task] Metamemory background processing completed in ${processingTime}s`,
        );
    ...
    .catch((error) => {
      ...
      if (!isAborted) {
        console.error(
          `[Task] ${isTimeout ? 'Timeout' : 'Error'} in metamemory background processing:`,
          error,
        );
      }
    })
...
async function processMetaCognition(
  ...
  if (taskLocalState.cognition?.processing) {
    ...
    if (processingTime > STUCK_THRESHOLD) {
      console.warn(
        `[Task] Meta cognition processing appears stuck (running for ${Math.round(processingTime / 1000)}s), forcibly resetting`,
      );
  ...
  console.log(
    `[Task] Processing meta cognition with ${metaState.unprocessedCognitionMessages.length} unprocessed messages`,
  );
  ...
  const cognitionPromise = spawnMetaThought(
    agent,
    messages,
    new Date(startTime),
    taskLocalState.requestCount || 0,
    taskLocalState,
  );
  const timeoutPromise = new Promise<never>((_, reject) => {
    const timeoutId = setTimeout(
      () =>
        reject(new Error('Meta-cognition processing timeout after 3 minutes')),
      COGNITION_TIMEOUT,
    );
  ...
  const cognitionOp = Promise.race([cognitionPromise, timeoutPromise])
    .then((result) => {
      ...
      console.log(
        `[Task] Meta-cognition background processing completed in ${processingTime}s`,
      );
    })
    .catch((error) => {
      const isTimeout = error.message?.includes('timeout');
      ...
      if (!isAborted) {
        console.error(
          `[Task] ${isTimeout ? 'Timeout' : 'Error'} in meta-cognition background processing:`,
          error,
        );
      }
    })
...
> {
  ...
  const messages: ResponseInput = taskLocalState?.messages
  ...
  async function* taskGenerator() {
    ...
    if (!taskLocalState?.runIndefinitely) {
      ...
      const toolGuidance =
        'You must complete tasks by using the provided tools. When you have finished a task, you MUST call the task_complete() tool with a comprehensive result. If you cannot complete the task, you MUST call the task_fatal_error() tool with an explanation. Do not just provide a final answer without using these tools.';
    ...
    if (taskLocalState?.messages && taskLocalState.messages.length > 0) {
      ...
      if (hasSystemMessage) {
        ...
        console.log(
          '[Task] Cleared agent instructions to prevent duplicates when resuming',
        );
    ...
    taskLocalState.cognition.disabledModels = taskLocalState.cognition
      .disabledModels
      ? new Set(
          Array.isArray(taskLocalState.cognition.disabledModels)
    ...
    if (
      taskLocalState.cognition.processing &&
    ...
    if (taskLocalState.memory.state) {
      ...
      taskLocalState.memory.state = {
        topicTags:
          state.topicTags instanceof Map
            ? state.topicTags
            : new Map(Object.entries(state.topicTags || {})),
        taggedMessages:
          state.taggedMessages instanceof Map
    ...
    if (
      taskLocalState.memory.processing &&
    ...
    if (taskLocalState.memory.enabled) {
      ...
      if (taskLocalState.memory.state) {
        ...
        console.log(
          '[Task] Restored metamemory state with',
          taskLocalState.memory.state.topicTags?.size || 0,
          'topics and',
          taskLocalState.memory.state.taggedMessages?.size || 0,
          'tagged messages',
        );
      }
    }
      ...
      if (metaState.unprocessedMemoryMessages.length >= 10) {
        ...
        if (taskLocalState && !taskLocalState.memory?.processing) {
          processMetaMemory(
            ...
            asyncEventQueue,
            pendingAsyncOps,
            metaProcessingAbortController.signal,
          );
        }
        return;
      }
    ...
    try {
      ...
      let totalErrors = 0;
      const MAX_CONSECUTIVE_SAME_ERRORS =
        taskLocalState.errorHandling?.maxConsecutiveInitErrors ?? 3;
      const MAX_TOTAL_ERRORS =
        taskLocalState.errorHandling?.maxTotalErrors ?? 10;
      ...
      while (!isComplete) {
        ...
        if (taskLocalState.requestCount > 1) {
          ...
          if (delay > 0) {
            try {
              ...
              await runThoughtDelayWithController(
                taskLocalState.delayAbortController,
                delay,
              );
            } catch (error) {
        ...
        console.log(
          `[Task] Request count: ${taskLocalState.requestCount}, Meta frequency: ${taskLocalState.cognition.frequency}`,
        );
        ...
        console.log(
          `[Task] Starting ensemble request with ${messagesToProcess.length} messages`,
        );
        for await (const event of ensembleRequest(
          ...
          if (event.type === 'error') {
            currentErrorMessage =
              (event as any).error || (event as any).message || 'Unknown error';
            totalErrors++;
            ...
            if (
              consecutiveSameErrors >= MAX_CONSECUTIVE_SAME_ERRORS ||
          ...
          if (event.type === 'tool_done' && 'result' in event) {
            ...
            if (responseEvent.message) {
              ...
              metaState.unprocessedCognitionMessages.push(
        ...
        if (consecutiveSameErrors >= MAX_CONSECUTIVE_SAME_ERRORS) {
          console.error(
            `[Task] Same error repeated ${consecutiveSameErrors} times - stopping task`,
          );
          isComplete = true;
          taskCompleted = true;
          ...
          console.error(
            `[Task] Too many errors (${totalErrors}/${MAX_TOTAL_ERRORS}) - stopping task`,
          );
          isComplete = true;
          taskCompleted = true;
      ...
      if (pendingAsyncOps.size > 0) {
        console.log(
          `[Task] Waiting for ${pendingAsyncOps.size} async operations to complete...`,
        );
        ...
        const waitTimeoutPromise = new Promise<void>((resolve) =>
          setTimeout(() => {
            console.warn(
              '[Task] Timeout waiting for async operations, continuing...',
            );
            resolve();
          }, MAX_WAIT_TIME),
        );
...
export function internalAddMessage(
  ...
  if (
    ...
    throw new Error(
      'Message must have a valid role: system, user, assistant, or developer',
    );
  }
...
export function addMessageToTask(
  ...
  if (!messages) {
    throw new Error(
      'Task not found or already completed. Messages can only be added to active tasks.',
    );
  }
